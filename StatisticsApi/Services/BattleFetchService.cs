        ...
        public async Task<BattleStatsOutputDto> GetBattleStats(int versionId = 0)
        {
            ...
            foreach (var instance in instances)
            {
                ...
                if (instanceRecords.Count != 0)
                {
                    ...
                    var inactiveBattles = instanceRecords.Where(b => b.CharacterResting != -1).ToList();
                    if (inactiveBattles.Count != 0)
                    {

                        instanceDto.AverageDamageTakenWhenInactive = inactiveBattles.Average(b => GetDamageTaken(b, b.CharacterResting));
                    }
